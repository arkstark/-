...
// 用于保存转换计算后的电压值	 
__IO float ADC_ConvertedValueLocal[8];
// AD转换结果值
uint32_t ADC_ConvertedValue[8];
...
void ADC_DRDL (void)
{
  ...
  DR = ADC_ConvertedValueLocal[1]*100/3.3-50; //右侧主动缸位移，单位mm                                     
  DL = ADC_ConvertedValueLocal[2]*100/3.3-50; //左侧主动缸位移，单位mm
  
  Dr_Butter = Butterworth(&DR_Butter,DR);
  Dl_Butter = Butterworth(&DL_Butter,DL);
